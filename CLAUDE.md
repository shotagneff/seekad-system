...
- **集計の定義（`approach-types.ts` の `aggregateActions`）**: 「回数」＝結果を初期値以外に変えた操作の数（同じ会社に2回電話すれば2）。「段階」＝その段階に到達した会社数（重複なし。送信済み→返信ありと2回記録しても送信1社・返信1社）。**率はすべて会社数どうし**。回数で割ると同じ会社に何度も電話するほど率が下がる
