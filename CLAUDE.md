...
### 2026-09-15（アポ獲得管理のリードのフェーズに「受注」を追加）

- **きっかけ**: リード管理のフェーズに受注が無く、受注した相手も「案件化済」のままでリード一覧から見分けられなかった。案件管理と同じく受注を選べるようにしてほしいという依頼
- **動き**: `LEAD_PHASES` に「受注」を追加（案件化済の次）。リードを「受注」にすると、案件が無ければ作ったうえで案件を受注にする。受注日・受注確度100%・顧客の作成は案件管理で受注にしたときと同じ `updateDeal` に任せる（`updateLead`、`src/lib/sales.ts`）。二重には作らない
- **逆向き**: 案件管理で受注にしたら、元のリードが「案件化済」なら「受注」に揃える。失注・協業など人が別の意味で付けたフェーズは上書きしない
- **画面**: リード表の絞り込みに「受注」を追加。「追いかけ中」からは受注を外す。受注の色は、黄色（案件化済）の先だと分かるようシアン（濃いめの水色）にした。案件管理の受注も同じ色に揃えた。赤は失注に見えるため不採用。初回面談・提案の水色（sky）とは別の色を `TONE.cyan` / `FILL.cyan` として足した
- **既存データは触っていない**: この変更より前に案件管理で受注にした相手のリードは「案件化済」のまま
